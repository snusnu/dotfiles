...
customize_macos_host() {
  ...
  sudo scutil --set ComputerName "mungo"
  sudo scutil --set HostName "mungo"
  sudo scutil --set LocalHostName "mungo"
  sudo defaults write /Library/Preferences/SystemConfiguration/com.apple.smb.server NetBIOSName -string "mungo"
  ...
  sudo systemsetup -setharddisksleep 10 > /dev/null
  sudo systemsetup -setcomputersleep 30 > /dev/null

  sudo pmset -a hibernatemode 3 displaysleep 10 disksleep 10 sleep 30
...
customize_macos() {
  ...
  defaults write com.apple.screensaver askForPassword -int 10
  ...
  # Enable dock item magnification
  ...
  # Delete all current persistent apps in the dock, then add the ones installed by brew cask
  ...
  # Use versioned iterm2 preferences
  defaults write com.googlecode.iterm2 PrefsCustomFolder -string "${DOTFILES_DIRECTORY}/macos"
  ...
  # Download blocklists and make them available to transmission
  # Transmission will parse these automatically on startup
  rm "$HOME"/Library/Application\ Support/Transmission/blocklists/*
  curl -Ls "http://list.iblocklist.com/?list=ydxerpxkpcfqjaybcssw&fileformat=p2p&archiveformat=gz" | gunzip > "$HOME"/Library/Application\ Support/Transmission/blocklists/level_1
  curl -Ls "http://list.iblocklist.com/?list=gyisgnzbhppbvsphucsw&fileformat=p2p&archiveformat=gz" | gunzip > "$HOME"/Library/Application\ Support/Transmission/blocklists/level_2
  curl -Ls "http://list.iblocklist.com/?list=uwnukjqktoggdknzrhgh&fileformat=p2p&archiveformat=gz" | gunzip > "$HOME"/Library/Application\ Support/Transmission/blocklists/level_3
  curl -Ls "http://list.iblocklist.com/?list=llvtlsjyoyiczbkjsxpf&fileformat=p2p&archiveformat=gz" | gunzip > "$HOME"/Library/Application\ Support/Transmission/blocklists/spyware
